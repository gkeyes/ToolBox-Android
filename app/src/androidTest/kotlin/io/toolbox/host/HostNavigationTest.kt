...
class HostNavigationTest {
    ...
    fun freshProductionCatalogNavigatesToImportReviewAndSettingsWithoutPickerLaunch() {
        ...
        composeRule.onNode(hasTestTag(HostTestTags.ImportFab) and hasText("导入")).performClick()
        composeRule.mainClock.advanceTimeBy(1_000)
        composeRule.waitForIdle()
        ...
        composeRule.waitForIdle()
        composeRule.waitUntil(timeoutMillis = 5_000) { toolsHeading.isDisplayed() }
        val settingsDestinations = composeRule.onAllNodesWithTag(HostTestTags.BottomSettings)
        settingsDestinations[settingsDestinations.fetchSemanticsNodes().lastIndex].performClick()
        composeRule.mainClock.advanceTimeBy(1_000)
        composeRule.waitForIdle()
