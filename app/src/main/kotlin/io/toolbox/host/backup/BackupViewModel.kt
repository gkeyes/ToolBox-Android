...
internal class BackupViewModel(private val operations: BackupOperations, private val documents: BackupDocumentIO) : ViewModel() {
    ...
    fun prepareExport() = runOperation {
        ...
        mutableState.value = BackupUiState.ExportReady
    }
    ...
    fun exportSelected(location: String?) {
        ...
        runOperation {
            ...
            try {
                ...
                mutableState.value = checkNotNull(publishedResult)
            } finally { withContext(NonCancellable + Dispatchers.IO) { cleanupExport() } }
        }
    }
    fun restoreSelected(location: String?) {
        ...
        runOperation {
            ...
            preview = found
            mutableState.value = BackupUiState.ConfirmRestore(found)
        }
    }
    fun confirmRestore() = runOperation {
        ...
        try {
            ...
            mutableState.value = BackupUiState.Result("恢复已完成", "已恢复宿主设置及 ${selected.recoverable.size} 个工具，跳过 ${selected.tools.size - selected.recoverable.size} 个不兼容工具。请重新打开工具检查数据。", warnings)
            cleanup()
        } catch (failure: BackupException) {
            if (failure.code != "PREVIEW_CHANGED") throw failure
            val refreshed = operations.refresh(selected)
            preview = refreshed
            mutableState.value = BackupUiState.ConfirmRestore(refreshed.copy(warnings = refreshed.warnings + backupMessage(failure)))
        }
    }
    fun cancel() {
        if (busy) {
            ...
            cleanup()
            mutableState.value = BackupUiState.Result("已取消", "未执行恢复，临时文件已清理。")
        }
    }
    ...
    private fun runOperation(action: suspend () -> Unit) {
        ...
        job = viewModelScope.launch {
            try { action() }
                ...
                withContext(NonCancellable + Dispatchers.IO) { cleanup() }
                mutableState.value = publishedResult ?: if (restoring && operations.committed) {
                ...
                withContext(NonCancellable + Dispatchers.IO) { cleanup() }
                mutableState.value = BackupUiState.Result("操作未完成", backupMessage(failure), failed = true)
            }
        }
    }
