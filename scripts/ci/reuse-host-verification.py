...
def main():
    ...
    if checks not in ("full", "performance"):
        raise ValueError("Unsupported host verification scope")
    outputs = {"scope": "current_run", "source_run": os.environ["GITHUB_RUN_ID"], "base_commit": os.environ["GITHUB_SHA"], "checks": checks}
    if run_id:
        ...
        commit, checks = validate_prior_run(run, jobs, repository, os.environ["DEFAULT_BRANCH"])
        ...
        outputs = {"scope": "reused_unchanged_host", "source_run": run_id, "base_commit": commit, "checks": checks}
        with Path(os.environ["GITHUB_STEP_SUMMARY"]).open("a") as handle:
            handle.write(f"Host inputs match merged commit `{commit}`. Reusing its completed `{checks}` checks from [run {run_id}](https://github.com/{repository}/actions/runs/{run_id}); this run checks only CI evidence and signed release startup.\n")
