...
if [[ "${1:-}" == "--case" ]]; then
    requested_case="${2:-}"
    [[ -n "$requested_case" ]] || {
        printf '%s\n' 'USAGE: self-test.sh [--case valid|missing-action-log|missing-ui-tree|missing-cleanup-receipt]' >&2
        exit 2
    }
    shift 2
fi
...
write_receipt() {
    ...
    local cleanup_value="$4"
    {
        printf '%s\n' \
            ...
            'candidate_sha=13c59aed9c3fd53fcdcd70079335602531390930' \
...
run_case() {
    ...
    case "$case_name" in
        valid)
            write_receipt "$receipt" "$action_log" "$ui_tree" "$cleanup_receipt"
            "$validator" "$receipt"
            ;;
        missing-action-log)
            write_receipt "$receipt" "$scratch_dir/missing-actions.txt" "$ui_tree" "$cleanup_receipt"
            "$validator" "$receipt"
            ;;
        missing-ui-tree)
            write_receipt "$receipt" "$action_log" "$scratch_dir/missing-ui.xml" "$cleanup_receipt"
            "$validator" "$receipt"
            ;;
        missing-cleanup-receipt)
            write_receipt "$receipt" "$action_log" "$ui_tree" "$scratch_dir/missing-cleanup.txt"
            "$validator" "$receipt"
            ;;
        *)
...
run_case valid
for invalid_case in missing-action-log missing-ui-tree missing-cleanup-receipt; do
