...
| 权限与运行时 API | `PermissionCenterViewModelTest`；`RuntimeRpcDispatcherTest` | 防止“有开关没能力”或跳过声明、授权、系统权限、手势和配额层；同时证明 M1/M2/M3 处理器不是伪成功。 | 对已支持 capability 用 production dispatcher 分别关闭 declaration、grant、system、gesture、quota 条件；同时验证权限列表写入、未知权限拒绝、文件一次性令牌和位置错误映射。 | 全部条件满足时得到真实 handler 结果；任一条件缺失返回稳定错误；未知权限不产生写入；文件令牌只能消费一次。 |
| Bridge | `HardenedRuntimeWebViewInstrumentationTest` | 保护导入内容的来源与会话边界。 | API 35 WebView 测试 exact origin、main frame、nonce、iframe、导航后的旧 nonce 和取消。 | 仅当前主 frame/来源/会话可调用；其他请求被拒绝且无副作用。 |
...
| 真机组合 | `manual-xiaomi-toolbox-v1` | 模拟器不能证明 SAF、相机、Sharesheet、快捷方式、通知和 HyperOS 系统栏。 | 干净安装后导入三例，依次测试权限、复制、文件、相机、定位、后台、删除并保存截图/日志回执。 | 实际系统 UI 可完成；工具内容优先；无明显滚动/切换卡顿。 |
...
| CI 交付 | `artifact-gate-receipt` | 防止测试未过就发布 APK。 | Actions 按 verify → emulator → artifact 的 `needs` 关系运行，并检查 APK、三个 `.tbx`、SHA256 和提交回执。 | 任一前置失败时没有 APK；成功产物可按 SHA256 复验。 |
