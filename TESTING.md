...
- 截图基准同步：`04ddb7c` 的 Android CI `34030584071` 已通过协议、安全、编译及准入单元测试，17 张截图仅设置浅色与中等屏设置深色失败。逐张查看该次原始图、实际图和差异图，确认仅版本文字 `0.3.8 → 0.3.10` 改变；其余 15 张渲染与既有基准逐字节相同。只采用这两张 GitHub 实际渲染更新相应 reference，生产页面、预览夹具、比较阈值及其余基准均不变。下一提交重新运行完整截图门禁；此处不将基准同步或离线预览记为 Android 真机通过。
