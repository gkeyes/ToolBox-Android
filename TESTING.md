...
| 后台 | `BackgroundTaskRepositoryTest`；`BackgroundTaskPolicyTest`；`RuntimeReminderPolicyTest`；`LiveNotificationCoordinatorTest`；`manual-xiaomi-toolbox-v1` 的 0.3.1 后台步骤 | 同时保护冻结的 WorkManager 任务语义、持续 runtime 的 12 小时提醒和实时展示生命周期，不把旧任务限制套到新会话。 | JVM 合同测试验证旧任务 RUNNING→QUEUED 恢复、三次退避和七天结果清理；可控时间输入验证首次与重复提醒都恰好相隔 12 小时；fake clock/刷新回调验证 live start 幂等、500ms 合并、最近状态选择、更新与清理；真机启动持续会话，离开/返回页面、等待行情更新、停止并检查前台通知和位置监听释放。 | 旧任务状态与重试不变；提醒按 12 小时重复；500ms 内只刷新一次 SystemUI 且最新值不丢，停止后无实时状态；持续页面重新挂载同一环境，停止后 WebView、timer、后台位置和通知均释放。 |
...
| 真机组合 | `manual-xiaomi-toolbox-v1` | 自动门禁不能证明 WebView detach/reattach、后台位置、精确闹钟、前台服务、Android 实时更新、HyperOS 超级岛和系统回收恢复；导航丝滑度也依赖真实设备。 | 安装 0.3.1 后验证三例与原能力；导入行情哨兵，开启通知和外部白名单模块，启动监控、退到后台、锁屏、等待更新并停止；同时覆盖 timer/location/alarm、重启恢复和关键进入/返回路径帧数据。 | 同一 runtime 状态连续，恢复事件不丢；通知原位显示股票、价格和更新时间；普通通知始终存在，超级岛仅在系统实际支持时增强；停止/授权关闭/更新/删除无残留；关键页面无可见停顿或残影。 |
...
| 行情哨兵打包 | `examples/stock-monitor/package.sh` 可重复性检查 | 确保独立 `.tbx` 可复验且只调用 0.3.1 已公开接口。 | `node --check` 后连续打包两次并比较 SHA-256，检查 manifest、integrity、ZIP 内容及源码不含 `notifications.focus`。 | 两次哈希一致；版本为 1.1.0 (2)、`minHostVersion=0.3.1`；ZIP 只含声明文件；实时展示使用 `notifications.live`。 |
| CI 交付 | `artifact-gate-receipt` | 防止协议/安全/编译/最小测试未过就发布 APK/TBX，也防止把未执行的设备测试写成通过。 | Actions 按 verify → delivery 的 `needs` 关系运行，检查 `toolbox-v0.3.1-debug.apk`、`stock-monitor-v1.1.0.tbx`、`SHA256SUMS.txt`、提交回执及 `DEVICE_TEST_RESULT=NOT_RUN_USER_OWNED`、`HYPEROS_ISLAND_TEST_RESULT=NOT_RUN_USER_OWNED`。 | 任一静态门禁失败时没有交付产物；成功 APK/TBX 来自同一提交并可按 SHA256 复验，回执明确真机与超级岛验证由用户执行。 |
