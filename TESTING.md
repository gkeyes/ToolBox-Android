...
| 后台 | `BackgroundTaskRepositoryTest`；`BackgroundTaskPolicyTest`；`RuntimeReminderPolicyTest`；`LiveNotificationCoordinatorTest`；`RuntimeNotificationRegressionTest`；`BackgroundTasksPresentationTest`；`manual-xiaomi-toolbox-v1` 的 0.3.2 后台步骤 | 同时保护冻结的 WorkManager 任务语义、持续 runtime 的 12 小时提醒和实时展示生命周期；专门防止返回宿主页后通知消失、深色锁屏文字不可读和持续会话被后台任务页漏掉。 | JVM 合同测试验证旧任务状态与提醒；fake clock 验证 live 合并与清理；回归测试验证 runtime detach 保留宿主并刷新通知、Focus 明暗两套文字字段均为白色、后台页按工具合并持续会话；真机启动持续会话，返回宿主页、锁屏、再进入后台任务页并停止。 | 旧任务语义不变；返回宿主页后持续通知仍存在；实时通知在浅色与深色主题下都使用白字；后台任务页显示并可停止当前工具的持续运行会话；停止后 WebView、timer、位置和通知均释放。 |
| 实时通知文字 | `RuntimeLiveNotificationRendererTest` | 防止只修正 Focus 内容却遗漏准备中、恢复占位和多会话摘要，或文字颜色在系统通知传递时丢失。 | 使用 production renderer 构建准备、恢复、单会话实时、多会话摘要四种通知，经过 Android Parcel 往返后检查标题、折叠正文、展开正文、辅助文字和动作标签的全长白色 span；不发送通知或启动后台服务。 | 即使工具强调色为黑色，以上文字仍携带白色；通知内容和操作不变。该测试只证明通知数据，不替代 HyperOS SystemUI 的真机显示验收。 |
