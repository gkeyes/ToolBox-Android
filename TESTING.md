...
| 测试 | 理由 | 方法 | 预期 |
|---|---|---|---|
| `ToolBoxContrastTest` | 小字号次级文字与暗色/危险按钮原有对比度不足。 | JVM 使用生产色值/对比度函数检查浅深两套按钮与三种文字底色；RGB 网格模拟动态按钮底色，验证低对比前景修正。 | 10 组固定关键配色均至少 4.5:1；动态单底色按钮保留合格前景，否则使用合格黑/白前景。禁用状态不冒充正常正文合规。 |
...
仓库已保存旧配色的 PNG reference，本轮未伪造/改写这些截图，也没有放宽截图阈值或跳过门禁。
颜色改变预期需要在 Android 构建环境执行 `./gradlew --no-daemon :app:updateDebugScreenshotTest`，
人工复核新的浅/深/大字体图像后提交 reference，再执行 `:app:validateDebugScreenshotTest`。
现有 CI 会上传实际渲染与差异报告；本 UI 提交在完成该步骤前不是通过视觉验收的 release 候选。
