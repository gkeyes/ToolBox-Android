...
- 权限页改用现有 Lifecycle 2.11 的局部 ViewModel owner；保留返回动画完成后的移除时机以及配置
  重建保留规则。页面的目录/grant 订阅与结果等待随页面释放，不改变运行环境、后台任务或权限规则。
- 用户已提交的有限保存与清理由应用级 runner 执行，同工具跨页面串行，其他工具不互相阻塞；
  不保存 UI 对象或历史结果，队列完成后移除引用。先提交再等待，返回不会取消已接受操作。
- 系统回调按唯一请求号、版本和实际请求权限匹配；空或缺失结果拒绝，旧结果不能授权新页面。

| 自动检查 | 理由 | 方法 | 预期 |
|---|---|---|---|
| `PermissionMutationRunnerTest` | 页面销毁不能取消已确认保存，重新进入不能绕过同工具清理顺序。 | 可控写入/清理和协程调度器；取消页面等待，排入同工具第二次操作及另一工具操作，检查顺序和队列释放；覆盖版本/声明变更、仓库冲突、重复值和安全清理失败。 | 同工具 FIFO、不同工具独立；撤权先写 false 再清理；清理失败不重新开启；旧版本/未声明拒绝；完成后无队列残留。 |
| `PermissionCenterViewModelTest` 扩展 | 页面离开应停止无用订阅，而不是中断已接受修改；系统回调不能串页或误授予。 | 生产 VM 放入 `ViewModelStore` 后清空；计数目录/grant 订阅，阻塞已提交写入再恢复；以新订阅模拟界面重新绑定，传入旧/错请求号、空/遗漏结果、粗略定位，以及工具更新/新 VM 后的回调。 | 订阅归零，已接受写入和副作用完成，旧 UI 不再更新；有效请求只保存一次，其他回调无越版本或跨页授权。不是 Android 弹窗/旋转实测。 |
| `HostNavigationTest` 扩展 | 真实导航重进与配置重建需要有原生验证入口。 | 在既有权限开关旅程中返回、重新进入、Activity 重建，再核对已保存网络权限，继续原运行/删除旅程。 | 重进/重建后的权限仍正确，返回正常；本轮只由 GitHub 编译该源码，原生执行交用户，不宣称通过。 |

- 本地只执行差异检查、安全不变量和门禁脚本语法检查。Kotlin、准入 JVM 用例和优化编译只在
  GitHub 执行，包含两个权限测试类；首次提交时结果待 CI。LSP 服务不可达，不以静态扫描冒充编译。
- 按用户最新分工，真机体验由用户手动验证，本轮不操作设备、不恢复截图测试，不把手动采集作为
  开发前置条件；帧率、耗时、配置重建实际交互仍为 NOT_MEASURED / NOT_RUN，源码优化不等于实测提速。
...
- 当前基准是默认分支 `0224623089c06c70c2fc97066050ecae3fa11f60`，
  [CI 33984763500](https://github.com/gkeyes/ToolBox-Android/actions/runs/33984763500) 三项 job 成功。
  用户要求继续执行有用修改、不等待真机；本批只改 F1，不混改图标、runtime、数据库或 UI。
- 修改：先发导航，再做可取消的统计写入；同工具尚未消费的请求合并；消费时拒绝目录已知删除的工具。
  统计按请求时间串行提交，避免旧写入覆盖较新时间；typed failure/异常仅产生 `RECENT_UPDATE_FAILED`，
  下次成功清除该类提示，取消不伪装失败。冷启动待目录、工具 ID 导航、运行准备/permit 安全边界保持。
- 前后证据：源码先后关系与下列确定性测试；不是点击延迟/帧率测量。真实运行准备、WebView 生命周期、
  双击触屏与前后性能仍 NOT_RUN / NOT_MEASURED；截图测试保持移除。
- 本地 `git diff --check`、`bash scripts/verify-security-invariants.sh` 通过。
  第三方 Kotlin 语法筛查：测试文件无报错，生产 VM 两处既有语法解析限制未增加；这不是 Kotlin 编译。
...
- 命令：`node --test examples/health-records/tests/*.test.mjs`。保留既有 41 项用例，新增 18 项有界回归，只使用合成记录、模拟存储和服务响应；不修改私人档案或读取真实密钥。
- `save-safety.test.mjs`（6 项）：理由是保存排队前的版本检查、可编辑的待保存表单及导出年份的异步读取会丢失数据或误标文件。方法是实际 store/应用事件配合可暂停的 head 写入及文件 Worker 替身。预期：过期 mutator 在队列内被拒绝且不执行，后续合法保存不受影响；档案、导入和返回/关闭/Escape 在提交时锁定，失败保留输入并可重试；即使外部强制关掉旧导入弹窗，新编辑器也不被晚到提交覆盖；JSON/Excel 的范围与文件名固定，改变范围作废预生成结果。
- `name-safety.test.mjs`（6 项）：理由是数量符号、显式方法、整组名称映射及分类证据不能只依赖 AI 自述。方法是在实际目录、匹配器、建议校验和批量应用函数中回放全角 `%/#`、中英文方法及空格/下划线/冒号、已有 scoped/legacy 别名与等价键、单向/双向/三节点跨单位映射、原始快照改名及重复冲突；分类回放空理由、虚构项目、不存在/重复证据 ID、不能支持目标类别的项目和有效证据。预期：未知或冲突不匹配，不新增循环规则；同标本单向跨单位同义匹配保留，血尿隔离；批量失败零局部修改；分类仅发送临时 ID、名称、单位，展示真实依据且不改记录结构。OCR 仍按来源标本/单位保存经确认的别名。
- `zip-safety.test.mjs`（6 项）：理由是 ZIP 目录的声明大小可伪造，Worker 超时不能替代内存预算。方法是在实际文件 Worker 入口构造不超过 700 KiB、实际膨胀为 16 MiB 的有界假 XLSX；监测原生流输入及实际输出，覆盖 8/12 MiB 精确边界、下一字节、CRC/损坏/尾部垃圾、数据描述符、未列出本地条目、正常 XLSX 与 `.xls` 往返及无原生解压支持。预期：SheetJS 调用前拒绝伪造与损坏数据，分块输入最大 128 字节、超预算即取消而不完整解压；普通备份与原文/零值保留，不支持时给出 WebView/JSON 备选。
- 手动浏览器回放实际导入、个人档案和 AI 建议保存的暂停/失败/重试、刷新读回、正常/伪造 Excel、按年导出、错误/有效分类依据、跨单位循环改名拒绝和 OCR 名称核对/日期保存。只有文件选择、存储时序和 AI 返回由仓库外适配器控制，页面及真实 Excel Worker 使用交付源码；适配器、合成工作簿及证据不进入 TBX，不等同于 Android、SAF、真实 AI 准确率或医疗安全认证。

### 健康档案 AI 逐条核对修复回归（1.0.9 候选，尚未交付）

- 命令：`node --test examples/health-records/tests/*.test.mjs`。保留 1.0.8 的 59 项测试；新增 38 项回归（当前合计 97 项）仅使用合成记录和模拟响应，不访问真实密钥或外部 AI。
- `ai-review.test.mjs`（11 项）：理由是单条无效建议曾导致整批有效建议被丢弃，复杂嵌套名称键易抄错，JSON 包裹解析未完整约束，资料章节及字段类型要求与验证器未对齐。方法：使用生产请求构造/解析、建议核对及实际页面事件，回放混合有效/无效、同来源重复、跨单位循环和已有 scoped/legacy 别名串联循环、有效/错误分类依据、空数组与全拒绝、短 ID 来源/目标校验（同名候选不能替代具体目标的标本和数量性质）、BOM/完整围栏与单边围栏/多 JSON/思考/截断/非法换行，以及重复键、转义等价重复键、无歧义嵌套字符串、非对象结果、12/14 章节和错误 OCR 字段类型。预期：只保留独立有效建议；血尿、方法、数量与比例边界不放宽；全部被拦截不显示“暂无建议”；默认不勾选，明确勾选后才保存，原数值/单位/参考/日期不变；损坏响应不猜测修补、重复字段不静默覆盖、OCR 不把 null/数值强制变为文本。
- `name-partial.test.mjs`（6 项）：理由是识别后 AI 对齐的一条错误对应会丢失其他正确对应。方法：生产匹配器回放未知目标、额外字段、数量冲突、重复来源及倒序、未知来源与本地命中共存、全拒绝/有效空结果、非数组/超过来源数的返回。预期：错误按条隔离、重复来源全拒绝、独立有效匹配保留；统计拦截数并保留错误行原名，所有非名称字段不变；结构及批次预算仍严格。
- `ai-stream.test.mjs`（6 项）：理由是普通 JSON 请求不能直接解析 SSE，MiniMax 实际首帧 finish_reason 可为空字符串且正常 EOF 可没有 DONE；生成额度、空正文与格式错误不能混为同一错误。方法：生产请求/解析器回放 1/7/1024 字节 UTF-8 分片、重复增量、双推理字段、空首帧、正常 stop/缺失 stop、length、空正文、JSON 服务错误、错误后 DONE、工具/拒绝响应、非法 UTF-8、4 MiB 上限、读中取消与迟到 open；核验 M3 思考生成额度为文档建议的 131072，等待仍 300000 毫秒。仅将测试进程中的 100 毫秒背压节流快进，不更改真实取消和截止时钟。预期：思考与最终 JSON 分离、不重发请求、不拼入诊断正文；截断/不完整/空结果/非法 JSON 分别有稳定错误码；取消后不接纳迟到结果。
- `ai-progress.test.mjs`（1 项）：理由是关闭等待框、替换草稿或 pagehide 后旧流不能继续更新界面/存储，AI 文本不可信。方法：实际等待组件回放关闭、取消、页面隐藏、替换与迟到旧 close，输入 HTML 形状的纯文字和显示截断标志。预期：只显示本次服务推理、不执行 HTML；取消一次、清空临时文字、移除监听/计时器，不持久化推理；旧 close 不取消新弹窗。
- `ai-format.test.mjs`（6 项）：理由是实际历史整理出现未转义引号，实际 MiniMax 工具参数续片会使用空 id/type/name 占位，且允许附带正文或不调用工具而正常结束。方法：检查六类程序生成的填写模板与参数 schema、预填来源 ID 和候选枚举；生产流式/非流式解析回放引号/换行/反斜杠、独立推理和附带正文、空续片、非法函数/第二调用/多 choice/变化 ID/截断、额外字段、无工具的完整模板 JSON/错误字段、明确留空的名称对应，以及 Gemini 的相同填写模板。预期：调用工具时只解析本次唯一声明的格式函数参数，不执行函数或解析附带正文；未调用工具的 stop 正文必须是完整 JSON 且符合相同模板；损坏参数不猜测修补，不强制填未知名称；模板字段严格检查，建议按条隔离错误；不增加未经文档确认的 response_format/tool_choice/strict 参数。保留 UI 测试全部断言，仅将 MiniMax 模拟响应改为实际结构化参数形状，Gemini OCR 不再夹带模板外的本地记录 ID。
- 手动验证使用隔离浏览器和合成存储，AI 返回由包外适配器控制；检查摘要核对保存、混合建议选中保存、全拒绝原因、刷新读回和识别后逐项对齐，不等同于真实 AI 准确率或 Android 验收。真实 MiniMax 调用的资料范围、结果及授权边界单独记录于 `output/health-records/verification-1.0.9/`，不得把合成服务成功写成真实私人档案已通过。

#### 1.0.9 最新用户规则：格式检查与内容判断分离、完整历史分组

- 用户明确撤销本地医学白名单、逐字理由及方法/数量启发式对 AI 内容的否决。此规则覆盖上文 `name-safety`、`ai-review`、`name-partial` 与既有 `ai`、`names` 用例中“方法/数量或本地目录未覆盖即拒绝”的旧预期：保留相同输入矩阵，改验建议与原理由照常展示、方法/数量差异提示核对、确认后的别名可保存。血尿隔离、实际 ID、字段/容量、循环、重复写入、原值不变和显式勾选等断言仍保留。分组预算测试用完整同标本目录回放，不再通过方法过滤缩减候选。
- `ai-content-review.test.mjs`（3 项）：理由是程序曾把不在硬编码名单内或理由未逐字复述的建议丢弃。方法：实际分类/名称校验、OCR 匹配、别名应用回放未知名称、方法和比例差异、任意非空理由，以及虚构/重复证据、未知类型、跨血尿和空字段。预期：内容不被白名单否决，原理由与真实引用并列保留，无法应用的原提案可查看；只有格式/记录对应及既有写入边界被拒绝，未确认不修改任何记录。
- `history.test.mjs`（5 项）：理由是整份真实历史流式请求反复超过 5 分钟。方法：逐行重建源数据与分组负载比较（含同日重复、空单位、血尿、方法和参考原文）；回放长序列分段/前点衔接、预算拒绝、第二组失败续接、取消/过期页面及每批完整单章节模板。预期：每条原始结果完整保留、仅首组含个人档案、最多 12 组且每批不超过明确预算；重试不重复发送已完成组；取消后不启动下一组或接纳晚到结果；全批完成才合成，合成无需额外 AI 调用，最大结果适配既有 30000 字符保存限额。
- 手动浏览器新增完整流程：确认全部行数/组数和额外费用 → 实时进度 → 第二组模拟失败 → 显式只重试未完成组 → 合成所有章节 → 保存 → 刷新读回；另验先前因本地医学名单/方法提示而被拒绝的建议现在可见、默认不勾选、只有勾选内容写入。合成服务与真实 MiniMax 分开报告，不声称医学内容已被验证。
