...
- 启动依赖加载/错误页跟随系统浅深主题；设置加载后仍使用用户保存的六种宿主主题之一。
  不为提前知道主题同步读取存储；原生启动窗口及自定义主题加载前的差异不在此源码修改中宣称解决。
...
设置使用“外观 / 运行与权限 / 支持”三组：主题；后台保障与工具权限；Developer Help 与关于。
不保留重复的页面副标题或常驻绿色“自动保存”提示。当前主题在行尾显示，说明只在现有主题选择
对话框出现；选项、系统/固定/动态取色与持久化语义不变，字体增大时文字自然换行。
保存失败仍在设置页显示，主题选择和其他入口保持可用，不把未成功保存写成成功。关于版本来自
