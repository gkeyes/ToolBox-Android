...
请求与 manifest 的 `timeoutMs` 都必须在 1000–3600000 毫秒内，最终时限取两者较小值；默认仍为
30000 毫秒。60 分钟只是工具可声明的上限，不是默认等待时间；数值高于 600000 毫秒的工具须将
`minHostVersion` 提高到 0.3.11。连接建立仍最多 10 秒，域名、重定向、地址和大小限制不因长等待放宽。
