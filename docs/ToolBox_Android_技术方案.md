...
- 当前候选为 `0.7.3 (27)`，沿用 GitHub 固定签名，可覆盖安装，不清除工具、授权或设置。
...
权限页的 ViewModel 使用已固定 Lifecycle 2.11 的 composable-scoped owner：屏幕配置重建时保留，
返回转场完成、页面移出组合后释放目录/grant 订阅和结果等待。有限的权限修改由应用级
`PermissionMutationRunner` 在 IO 调度器执行，同一工具跨页面实例串行；提交后退出页面不取消
保存或清理，完成后移除队列引用。后台运行环境和真正的授权校验不归页面所有，也不随页面释放。
Android 授权请求绑定唯一请求号、版本和请求权限；页面只保存已启动的请求号。旧版本回调、
错误请求号、空结果或未包含请求权限的结果不能开启 grant，前台定位仍允许粗略或精确定位任一项。
...
不叠放宿主返回按钮，系统返回手势或按键先显示返回 ToolBox 的确认弹窗，确认后才退出工具；取消或关闭弹窗不卸载当前 WebView。
进入过渡壳共用此布局，不显示标题、刷新/更多或 origin/API/安全技术副标题。
...
从屏幕边缘向内滑动属于 Android 系统返回，用于请求退出当前小工具、回到 ToolBox，须在原生确认弹窗点击“返回 ToolBox”后执行；左边缘向右滑与系统支持的右边缘向左滑保持一致。小工具不得拦截或占用系统边缘返回区域，宿主不叠放悬浮返回按钮。
...
- 浅深两套主题的普通次级文字与主/危险按钮前景按 4.5:1 验证；危险按钮拥有独立 `onDanger`。
  Monet 保留合格颜色，仅在关键前景不合格时选择更可读的黑/白色，不增加模糊、阴影或额外动画。
...
使用最低忠实层和参数化矩阵，删除审计、审核、发布者和旧迁移功能的测试，不保留纯文案/占位测试。
...
- GitHub 只在静态门禁通过后发布同签名 release APK、SHA256 和同提交回执，不伪造设备验证结论。
