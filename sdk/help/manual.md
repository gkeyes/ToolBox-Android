...
宿主分为首页、全部工具和设置。首页顶部显示最近使用，下方是收藏与可展开分组；同一工具可同时属于多个分组。点首页顶部“整理”后，可长按拖动收藏、分组和组内工具；拖到内容边缘时列表会自动滚动，也可用无障碍操作中的“前移/后移”调整顺序。点“完成”或按返回键退出整理，切换标签也会退出。正常浏览时点工具图标打开，长按显示操作面板；整理时点图标进入操作面板，避免误打开。面板中的星标用于收藏，加入分组支持搜索及多选；下方“管理工具”进入详情、权限和卸载。分组点名称展开图标，长按名称编辑；整理时通过分组旁的更多按钮编辑。分组编辑可搜索工具，保存失败会保留草稿。删除组不会卸载工具。全部工具可搜索，并按名称（中文拼音）、首次安装时间或最后打开时间排序；更新不会重置首次安装时间。最后打开记录宿主接受打开请求的时间，不代表网页已加载完成。
