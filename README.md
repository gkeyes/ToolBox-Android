...
当前候选为 `0.6.3 (19)`，由 GitHub 使用同一签名密钥构建 release APK，可覆盖已交付版本，
...
0.6.3 增加按工具授权的自定义 HTTPS 域名，并提供独立 NextFlux 阅读器，包含 OpenAI 兼容 AI 摘要。
...
`toolbox-v0.6.3-release.apk`、`SHA256SUMS.txt` 和构建回执。构建会核对 APK 不可调试、固定签名、
