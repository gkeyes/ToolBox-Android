package io.toolbox.tool.packagekit.fixtures
...
/** Test-only repository fixture extracted from the former in-memory implementation.
 * Catalog updates, grants and install transaction state share one mutex; no production source
 * or storage limits are restored by this fixture.
 */
internal object InMemoryCoreData {
    fun create(initialSettings: HostSettings = HostSettings()): CoreDataRepositories {
        ...
        return CoreDataRepositories(
            ...
            lifecycle = InMemoryCatalogLifecycleRepository(state),
...
private class InMemoryHostSettingsRepository(initial: HostSettings = HostSettings()) : HostSettingsRepository {
...
private fun validateTask(task: BackgroundTask): DataResult.Failure? {
    return when {
        ...
        task.periodic && (task.intervalMinutes ?: 0) < 15 ->
...
private fun String.isValidTransactionId() = isNotBlank()
private fun String.isValidTaskId() = isNotBlank()
private fun String.isValidTaskKey() = isNotBlank()
