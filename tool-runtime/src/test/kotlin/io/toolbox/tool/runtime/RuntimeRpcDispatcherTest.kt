...
class RuntimeRpcDispatcherTest {
    ...
    fun legacyDomainMethodsRequireNetworkGrantAndSessionWithoutDomainOrGestureApproval() = runTest {
        ...
        val handler = object : RuntimeNetworkDomainHandler {
            override suspend fun authorizeDomain(domain: String): Boolean { authorized++; return true }
            override suspend fun listDomains() = emptyList<String>()
        }
        val policy = MutablePolicy()
        val declaredIdentity = identity.copy(declaredCapabilities = setOf("network"))
        ...
        policy.granted = false
        assertFailure(RuntimeRpcErrorCode.PERMISSION_DENIED, dispatcher().dispatch(rpc, inbound))
        assertFailure(RuntimeRpcErrorCode.PERMISSION_DENIED, dispatcher().dispatch(request(method = "network.listDomains", params = RpcValue.ObjectValue(emptyMap())), inbound))
        ...
        val list = dispatcher().dispatch(request(method = "network.listDomains", params = RpcValue.ObjectValue(emptyMap())), inbound)
        assertEquals(RpcValue.ArrayValue(emptyList()), (list as RuntimeRpcResponse.Success).result)
    }
