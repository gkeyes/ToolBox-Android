...
使用 4 MiB 网络响应与 8 MiB 消息上限容纳 GitHub 的完整构建列表；响应过大、超时、连接失败与域名阻止会显示不同错误，不把它们统一描述为断网。请先更新到 ToolBox 0.7.6 或更高版本，再更新工具包。正常同身份更新保留已保存 Token 和仍有效的权限选择；未签名更新需先在宿主确认，原来关闭的权限不会开启。旧宿主已清理的 Token 仍需重新填写。
...
Token 只写入宿主 `storage.secure`，不写入普通状态或网页本地存储；安全存储不可用会明确提示，不会假装保存成功。正常关闭、重新打开和后台恢复会重新读取已保存值；ToolBox 0.7.6 起，经身份校验或明确确认的更新会保留安全存储和原密钥。主动关闭安全存储权限或卸载工具仍会清理 Token，不要用关闭再开启权限来尝试恢复。
