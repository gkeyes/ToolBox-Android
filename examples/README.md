...
在 GitHub 打开“下载”链接后，选择 **Download raw file** 保存 `.tbx`，再到 ToolBox 的工具页导入。可以使用 ToolBox 0.6.5 导入下表各包；“最低 ToolBox”来自包内声明，不表示本轮重新验证了每个旧版本的运行表现。
...
| NextFlux | 1.0.3 | 0.6.5 | Miniflux 阅读、AI 摘要与同步 | [下载](packages/nextflux-v1.0.3.tbx) · [源码](nextflux/) |
...
- NextFlux 1.0.3 使用 [GitHub 构建产物](https://github.com/gkeyes/ToolBox-Android/actions/runs/34253878258)，恢复上游每页 1000 篇及完整同步范围，移除文章缓存淘汰；配套 ToolBox 0.6.5 移除持久存储总容量配额，并修复大数据存储。旧 [1.0.1](packages/nextflux-v1.0.1.tbx) 保留。四个内置示例沿用已通过 CI 的 ToolBox 0.6.4 包。
