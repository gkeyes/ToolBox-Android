...
这里集中收录本地小工具的源码与可导入安装包。主表列出收录版本；已交付版本与整理候选分开说明。原本的本地文件保留。

在 GitHub 打开“下载”链接后，选择 **Download raw file** 保存 `.tbx`，再到 ToolBox 的工具页导入。可以直接使用 ToolBox 0.6.4 导入下表各包；“最低 ToolBox”来自包内声明，不表示本轮重新验证了每个旧版本的运行表现。
...
| 健康档案 | 1.0.9（已交付） | 0.3.10 | 本地健康记录、趋势与可选 AI 整理 | [下载](packages/health-records-v1.0.9.tbx) · [版本说明](health-records/) |
| 稳力 | 1.0.2 | 0.6.4 | 离线节奏训练 | [下载](packages/kegel-trainer-v1.0.2.tbx) · [源码](kegel-trainer/) |
| NextFlux | 1.0.1 | 0.6.4 | Miniflux 阅读、AI 摘要与同步 | [下载](packages/nextflux-v1.0.1.tbx) · [源码](nextflux/) |
...
- 各小工具目录保留源码、说明和封装入口；健康档案的 `health-records/web/` 当前为 1.0.11 整理候选源码，并非 1.0.9 已交付包的源码快照。
...
- 健康档案以使用者确认的 1.0.9 为已交付版本。原本地 1.0.10 明确为未发布候选；[1.0.11](packages/health-records-v1.0.11.tbx) 是本次整理时对齐源码与最低版本要求后封装的候选，不作为已交付最新版。候选文件保留，未重新验证真实账号或设备。
- NextFlux 1.0.1 沿用 GitHub 构建产物；四个内置示例提取自已通过 CI 的 ToolBox 0.6.4 安装包。其余沿用已核对的现有包。
