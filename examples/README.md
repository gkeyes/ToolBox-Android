...
这里集中收录本地小工具的源码与可导入安装包。每个工具只列一个当前版本；旧文件保留在原本的本地目录，不重复上传。
...
- 各小工具目录保留源码、说明和封装入口；健康档案的运行文件位于 `health-records/web/`。
...
- 健康档案 1.0.11 将包与已有源码中的 ToolBox 0.6.1 要求对齐，记录及 AI 处理逻辑不变。
