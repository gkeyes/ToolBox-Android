...
async function setup(page:Page,{withProfile=true,withKey=true}:{withProfile?:boolean;withKey?:boolean}={}){
  await page.addInitScript(({profile,scene,withProfile,withKey,key,learnerLine,npcLine})=>{
    ...
    const secure=saved('test.native.secure',withKey?{'socialcoach.llm.v1':JSON.stringify({state:{enabled:true,provider:'openai',baseUrl:'https://mock.invalid/v1',apiKey:key,fastModel:'mock-chat',smartModel:'mock-assess',tokenParam:'max_tokens'},version:0})}:{});
    w.__testHost={ordinary,secure,calls:[],cancelled:0,exports:[],hold:false};
    ...
    w.ToolBox={ready:async()=>({hostVersion:'1.0.0'}),storage:{...storage(ordinary,'test.native.ordinary'),secure:storage(secure,'test.native.secure')},
      network:{openStream:async(req:any,{signal}:any={})=>{
        ...
        w.__testHost.calls.push({url:req.url,method:req.method});
        ...
        const payload=models?JSON.stringify({data:[{id:'mock-chat'},{id:'mock-assess'},{id:'text-embedding'}]}):sse?'data: '+JSON.stringify({choices:[{delta:{content:text}}]})+'\r\n\r\ndata: [DONE]\r\n\r\n':JSON.stringify({choices:[{message:{content:text},finish_reason:'stop'}]});
  ...
  },{profile,scene,withProfile,withKey,key,learnerLine,npcLine});
