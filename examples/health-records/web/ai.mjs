import { HealthError, buildIndex, TYPES, normalizeRecord, localDate } from "./model.mjs";
...
const reportRules = "按资料主题归纳；summary、title、text 必须是字符串；sections 必须是数组，每项是含 title 和 text 的对象。优先简洁陈述，不必逐条重抄完整数据。";
const jsonInstruction = "输出必须是单个完整的 JSON 对象，严格遵守当前操作的字段及类型。属性名用双引号，字符串内的换行、双引号和反斜杠必须按填写模板附带的范例正确转义。禁止注释、尾逗号、NaN、Infinity、重复键、JSON 外的解释文字或 Markdown 围栏。返回前检查所有引号、逗号和括号配对。suggestions 如有返回，必须是数组；每项 reason 是非空字符串；所有来源、目标、记录或证据 id 必须逐字复制输入。";
...
export const AI_MODES = {
  ...
  classify: { title: "检验类型整理", scope: "记录标识、当前检验类型、指标名称和单位，不含结果和病史", prompt: `仅提出分类修正建议，不跨标本推断。${classificationRules}可用类型 ${JSON.stringify(TYPES)}。按程序模板填写记录 id、建议类型、本记录中 不重复的证据项目 id，以及非空的分类理由；程序会并列显示所引用项目的原名与理由，由用户核对，不要求在理由里逐字抄写项目名。只输出你判断需要修正的记录，不直接修改记录；不能借用其他报告的项目，无法确定时说明限制或不提出修改。` },
};
...
export function getAiConfig(settings, withImage = false) {
  ...
  if (typeof model !== "string" || !/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(model)) throw new HealthError(`请在 AI 设置中填写有效的 ${definition.label} 模型名称`);
...
export function makeAiRequest(settings, key, prompt, payload, image = null, streaming = false, mode = null) {
  ...
  if (config.provider === "minimax") {
    ...
    request = {
      ...
      body: { systemInstruction: { parts: [{ text: systemInstruction + jsonInstruction }] }, contents: [{ role: "user", parts }], generationConfig: { responseMimeType: "application/json" } },
    };
  }
...
function boundedText(value) { return typeof value === "string" ? value : null; }

export function reviewSuggestions(value, archive, mode) {
  if (!["cleanup", "classify"].includes(mode) || !exactFields(value, ["suggestions"]) || !Array.isArray(value.suggestions)) throw new HealthError("AI 建议结构无效，未修改记录");
...
function evidenceNames(suggestion, record) {
  ...
  if (!Array.isArray(ids) || ids.length < 1 || new Set(ids).size !== ids.length) throw new HealthError("分类建议的引用格式需为本报告 不重复项目编号；原分类未修改", "AI_EVIDENCE_ID");
  return ids.map((id) => {
...
export function validateAiReport(value, limits = reportLimits) {
  if (!exactFields(value, ["summary", "sections"]) || !boundedText(value.summary) || !Array.isArray(value.sections) || value.sections.length < (limits.minSections || 0)) throw new HealthError("AI 摘要结构无效，未保存结果");
  const sections = value.sections.map((s) => {
    const title = boundedText(s?.title), text = boundedText(s?.text);
...
export function validateSuggestions(value, archive, mode) {
  if (!value || !Array.isArray(value.suggestions)) throw new HealthError("AI 建议结构无效，未修改记录");
  const index = buildIndex(archive.records), seen = new Set();
  const suggestions = value.suggestions.map((s) => {
    if (!s || boundedText(s.reason) === null || !s.reason.trim()) throw new HealthError("AI 建议缺少有效理由");
    if (mode === "cleanup") {
      ...
      if (!metric || !boundedText(s.target) || metric.name === s.target || !target) throw new HealthError("来源或目标不存在、名称未变或属于不同标本，无法应用；AI 原建议可查看");
...
export function validateOcr(value) {
  if (!exactFields(value, ["date", "type", "items"]) || typeof value.date !== "string" || typeof value.type !== "string" || !Object.hasOwn(TYPES, value.type) || !Array.isArray(value.items) || value.items.length === 0) throw new HealthError("AI 未识别出符合格式约定的检验表格，请重试或手动录入");
