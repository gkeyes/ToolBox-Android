...
export function createAiProgress({ api, config, dialog, isCurrent, onCancel }) {
  ...
  const elapsed = h("p", { class: "small muted" }, "已耗时 0 秒");
  ...
  const element = h("div", { class: "ai-progress" }, h("div", { class: "ai-progress-status" }, h("span", { class: "loading-indicator", "aria-hidden": "true" }), status), elapsed,
    h("details", { open: true }, h("summary", {}, streaming ? `${config.label} 实时推理` : "请求进度"), thinking),
  ...
  const timer = setInterval(() => {
    cancelIfObsolete();
    if (isCurrent()) elapsed.textContent = `已耗时 ${Math.floor((Date.now() - started) / 1000)} 秒`;
  }, 1000);
  return {
    element,
