...
面向 ToolBox 0.3.1 的独立股票监控小工具，不会加入宿主内置三个范例。
...
- 后台持续通知显示第一只启用股票的名称、价格、涨跌和更新时间，并原位请求 Android 实时更新与 HyperOS 超级岛增强。
...
产物写入 `build/examples/stock-monitor.tbx`，包内只包含 `manifest.json`、HTML、CSS、JavaScript、SVG 图标和生成的 `integrity.json`。
