...
export default function AddFeedModal() {
  ...
  const supportedTypes = [
    ...
    {
      id: "podcast", label: t("feed.podcast"), prefix: "", suffix: "", rewrite_rules: "",
      icon: <Podcast className="size-4 text-[#9933CC]" />, placeholder: t("feed.podcastPlaceholder"),
    },
    {
      id: "mastodon",
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchType === "podcast") {
        ...
        feeds = data.results.filter(item => item.feedUrl).map(item => ({ url: item.feedUrl, title: item.collectionName }));
      } else {
      ...
      toast.error(e.code === "PERMISSION_DENIED" ? "请先开启小工具的网络权限。" : "搜索失败，请检查输入后重试。");
