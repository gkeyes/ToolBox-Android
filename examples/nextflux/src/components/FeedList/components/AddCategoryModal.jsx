...
import { createCachedCategory } from "@/stores/syncStore.js";
...
export default function AddCategoryModal() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await createCachedCategory(title);
      onClose();
      toast.success(t("common.success"));
    } catch (error) {
      if (error.code !== "ACCOUNT_CHANGED") toast.error(error.message || "添加分类失败，请刷新后重试。");
    } finally {
      setLoading(false);
    }
  };
