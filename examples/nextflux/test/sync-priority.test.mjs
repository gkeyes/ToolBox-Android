...
test("incremental sync finishes unread before starting lower-priority requests", async () => {
  const events = [];
  let releaseUnread;
  ...
  const operation = runPrioritizedArticleSync({
    ...
    changed: async () => { events.push(["start", "changed"]); },
    fresh: async () => { events.push(["start", "fresh"]); },
  ...
  const startChanged = events.findIndex((event) => event[0] === "start" && event[1] === "changed");
  const startFresh = events.findIndex((event) => event[0] === "start" && event[1] === "fresh");
  assert.ok(doneUnread >= 0 && startChanged > doneUnread && startFresh > doneUnread);
});
...
test("initial sync waits for unread before read bookmarks", async () => {
  ...
  await runPrioritizedArticleSync({
    ...
    changed: async () => assert.fail("changed is incremental only"),
    fresh: async () => assert.fail("fresh is incremental only"),
  });
