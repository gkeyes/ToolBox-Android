...
5. **后台所有权清晰。** 运行环境由应用级管理器持有；只有网页主动启动持续会话时才脱离界面
   生命周期。每个会话独立通知卡，分别更新、打开和停止；一个前台服务承载，不增加全局提示卡。
   通知隐藏遵循手机默认机制，不由宿主暂停后台或要求手动恢复显示。
...
0.3.6 候选沿用同一签名覆盖安装，不清数据；交付 APK、SHA256 清单和与同一提交绑定的 GitHub Actions 回执；
四个现有范例继续内置但不作为独立产物。每个真实测试必须说明理由、方法、预期和实际结果；自动门禁不能替代 Xiaomi 真机
